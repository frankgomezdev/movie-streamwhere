...
const MovieCard = ({ movie }) => {
    ...
    useEffect(() => {
        axios.get(`api/movie/${movie.id}/watch/providers`)
        .then(response => {
    ...
    <div className="card-bordered bg-base-100 rounded-3xl min-w-0 text-neutral-500 hover:duration-100 hover:bg-neutral-200/40 transition-colors xl:aspect-[2/2.2] flex flex-col">
        ...
        <div className='flex-1 flex items-center justify-center'>
            <div className='max-h-64 h-full p-4 2xl:max-h-80'>
                <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt="" className='block h-full mx-auto rounded-2xl shadow-2xl' />
            </div>
        </div>
        <footer className='flex justify-between p-4'>
            <span>Streaming in {numberOfCountries} {numberOfCountries === 1 ? 'country' : 'countries'} </span>
            <a href="" className='text-black flex translate-x-3 transition-transform group-hover:translate-x-0 items-center'>See where</a>
        </footer>
</div>
  )
}
